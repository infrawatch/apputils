...
import (
    ...
    "github.com/vyzigold/apputils/connector/loki"
)
...
func TestLoki(t *testing.T) {
    ...
    client, err := loki.CreateClient(url, batchSize, maxWaitTime)
    if err != nil {
    ...
    t.Run("Test sending in batches", func(t *testing.T) {
        c, err := loki.CreateClient(url, batchSize, 10 * time.Second)
        if err != nil {
            t.Fatalf("Failed to create loki client: %s", err)
        }
    ...
    t.Run("Test waiting for maxWaitTime to pass", func(t *testing.T) {
        c, err := loki.CreateClient(url, batchSize, maxWaitTime)
        if err != nil {
            t.Fatalf("Failed to create loki client: %s", err)
        }
        labels := make(map[string]string)
